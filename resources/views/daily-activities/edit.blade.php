...
<head>
    ...
    <title>Edit Daily Activity</title>
            ...
            <div class="px-4 py-6 sm:px-0">
                <div class="bg-white shadow rounded-lg">
                    <div class="px-4 py-5 sm:p-6">
                        <form method="POST" action="{{ route('daily-activities.update', $dailyActivity) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="zone" id="zone" value="{{ old('zone', $dailyActivity->zone) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                    @error('zone')
                                ...
                                <div>
                                    ...
                                    <input type="text" name="pic_name" id="pic_name" value="{{ old('pic_name', $dailyActivity->pic_name) }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                    @error('pic_name')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="date" id="date" value="{{ old('date', $dailyActivity->date ? $dailyActivity->date->format('Y-m-d') : '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                    @error('date')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="abnormality" id="abnormality" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        <option value="n" {{ old('abnormality', $dailyActivity->abnormality) == 'n' ? 'selected' : '' }}>No</option>
                                        <option value="y" {{ old('abnormality', $dailyActivity->abnormality) == 'y' ? 'selected' : '' }}>Yes</option>
                                    </select>
                                    @error('abnormality')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    @if($dailyActivity->image)
                                        <div class="mt-1 mb-2">
                                            <img src="{{ asset('storage/' . $dailyActivity->image) }}" alt="Current Image" class="h-20 w-20 object-cover rounded">
                                        </div>
                                    @endif
