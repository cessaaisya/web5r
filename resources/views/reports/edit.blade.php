        ...
        <header class="bg-maroon shadow">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center">
                    <h1 class="text-3xl font-bold text-white">Edit Report</h1>
                        ...
                        <form method="POST" action="{{ route('reports.update', $report) }}">
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <select name="category" id="category" class="mt-1 block w-full py-2 px-3 border border-gold bg-white rounded-md shadow-sm focus:outline-none focus:ring-maroon focus:border-maroon sm:text-sm" required>
                                        ...
                                        @foreach($categories as $category)
                                            <option value="{{ $category }}" {{ $report->category == $category ? 'selected' : '' }}>{{ ucfirst($category) }}</option>
                                        @endforeach
                                    </select>
                                    @error('category')
                                ...
                                <div>
                                    ...
                                    <input type="text" name="metric_name" id="metric_name" value="{{ old('metric_name', $report->metric_name) }}" class="mt-1 block w-full py-2 px-3 border border-gold bg-white rounded-md shadow-sm focus:outline-none focus:ring-maroon focus:border-maroon sm:text-sm" required>
                                    @error('metric_name')
                                        <p class="mt-1 text-sm text-maroon">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="value" id="value" value="{{ old('value', $report->value) }}" class="mt-1 block w-full py-2 px-3 border border-gold bg-white rounded-md shadow-sm focus:outline-none focus:ring-maroon focus:border-maroon sm:text-sm" required>
                                    @error('value')
                                        <p class="mt-1 text-sm text-maroon">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="report_date" id="report_date" value="{{ old('report_date', $report->report_date ? \Carbon\Carbon::parse($report->report_date)->format('Y-m-d') : '') }}" class="mt-1 block w-full py-2 px-3 border border-gold bg-white rounded-md shadow-sm focus:outline-none focus:ring-maroon focus:border-maroon sm:text-sm">
                                    @error('report_date')
                                        <p class="mt-1 text-sm text-maroon">{{ $message }}</p>
                                    @enderror
                                </div>
