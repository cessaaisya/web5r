        ...
        <header class="bg-blue-600 shadow">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center">
                    <h1 class="text-3xl font-bold text-white">Edit List Finding</h1>
            ...
            <div class="px-4 py-6 sm:px-0">
                <div class="bg-white shadow rounded-lg">
                    <div class="px-4 py-5 sm:p-6">
                        <form method="POST" action="{{ route('list-findings.update', $listFinding) }}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')

                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    <label for="daily_activity_id" class="block text-sm font-medium text-gray-700">Daily Activity</label>
                                    <select name="daily_activity_id" id="daily_activity_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        <option value="">Select Daily Activity</option>
                                        @foreach($dailyActivities as $activity)
                                            <option value="{{ $activity->id }}" {{ old('daily_activity_id', $listFinding->daily_activity_id) == $activity->id ? 'selected' : '' }}>
                                                {{ $activity->zone }} - {{ $activity->pic_name }} ({{ $activity->date->format('Y-m-d') }})
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('daily_activity_id')
                                ...
                                <div>
                                    ...
                                    <select name="level" id="level" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        <option value="s" {{ old('level', $listFinding->level) == 's' ? 'selected' : '' }}>S</option>
                                        <option value="q" {{ old('level', $listFinding->level) == 'q' ? 'selected' : '' }}>Q</option>
                                        <option value="p" {{ old('level', $listFinding->level) == 'p' ? 'selected' : '' }}>P</option>
                                        <option value="c" {{ old('level', $listFinding->level) == 'c' ? 'selected' : '' }}>C</option>
                                        <option value="hr" {{ old('level', $listFinding->level) == 'hr' ? 'selected' : '' }}>HR</option>
                                    </select>
                                    @error('level')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    <label for="countermeasure" class="block text-sm font-medium text-gray-700">Countermeasure</label>
                                    <textarea name="countermeasure" id="countermeasure" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>{{ old('countermeasure', $listFinding->countermeasure) }}</textarea>
                                ...
                                <div>
                                    <label for="countermeasure_schedule" class="block text-sm font-medium text-gray-700">Countermeasure Schedule</label>
                                    <input type="date" name="countermeasure_schedule" id="countermeasure_schedule" value="{{ old('countermeasure_schedule', $listFinding->countermeasure_schedule ? $listFinding->countermeasure_schedule->format('Y-m-d') : '') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                    @error('countermeasure_schedule')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="progress" id="progress" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        <option value="pending" {{ old('progress', $listFinding->progress) == 'pending' ? 'selected' : '' }}>Pending</option>
                                        <option value="in_progress" {{ old('progress', $listFinding->progress) == 'in_progress' ? 'selected' : '' }}>In Progress</option>
                                        <option value="completed" {{ old('progress', $listFinding->progress) == 'completed' ? 'selected' : '' }}>Completed</option>
                                        <option value="cancelled" {{ old('progress', $listFinding->progress) == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    </select>
                                    @error('progress')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    @if($listFinding->image)
                                        <div class="mt-1 mb-2">
                                            <img src="{{ asset('storage/' . $listFinding->image) }}" alt="Current Image" class="h-20 w-20 object-cover rounded">
                                        </div>
                                    @endif
