...
<head>
    ...
    <title>Create List Finding</title>
        ...
        <header class="bg-blue-600 shadow">
            <div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between items-center">
                    <h1 class="text-3xl font-bold text-white">Create List Finding</h1>
            ...
            <div class="px-4 py-6 sm:px-0">
                <div class="bg-white shadow rounded-lg">
                    <div class="px-4 py-5 sm:p-6">
                        <form method="POST" action="{{ route('list-findings.store') }}" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    ...
                                    <select name="daily_activity_id" id="daily_activity_id" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        ...
                                        @foreach($dailyActivities as $activity)
                                            <option value="{{ $activity->id }}" {{ old('daily_activity_id') == $activity->id ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <select name="level" id="level" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        <option value="s" {{ old('level') == 's' ? 'selected' : '' }}>S</option>
                                        <option value="q" {{ old('level') == 'q' ? 'selected' : '' }}>Q</option>
                                        <option value="p" {{ old('level') == 'p' ? 'selected' : '' }}>P</option>
                                        <option value="c" {{ old('level') == 'c' ? 'selected' : '' }}>C</option>
                                        <option value="hr" {{ old('level') == 'hr' ? 'selected' : '' }}>HR</option>
                                    </select>
                                    @error('level')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <textarea name="countermeasure" id="countermeasure" rows="4" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>{{ old('countermeasure') }}</textarea>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="countermeasure_schedule" id="countermeasure_schedule" value="{{ old('countermeasure_schedule') }}" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                ...
                                <div>
                                    ...
                                    <select name="progress" id="progress" class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                                        ...
                                        <option value="cancelled" {{ old('progress') == 'cancelled' ? 'selected' : '' }}>Cancelled</option>
                                    </select>
                                    @error('progress')
                                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                    @enderror
